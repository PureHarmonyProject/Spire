...
spire_web 的依赖注入(DI)是其核心架构，提供轻量级且高效的实现，支持完整的生命周期管理。
...
以上实现展示了spire_web风格的核心功能模块，包括依赖注入、配置管理、日志系统和后台服务，这些模块协同工作为应用程序提供了坚实的基础设施支持。
...
参考源码：[HttpServer.cj](https://gitcode.com/spire/spire_web_hosting/blob/main/src/HttpServer.cj)
...
### mini-api
``` cangjie
//需要引入spire_web_routing模块
main(args: Array<String>): Int64 {
    let builder = WebHost.createBuilder(args)
    
    //spire_web_routing模块对容器的扩展
    builder.services.addRouting()
    
    let host = builder.build()
    
    //spire_web_routing模块对管道的扩展
...
* 我们可以引入spire_web_mvc模块实现controller-action-model的webapi开发模式

> 我们提供了大量的可插拔的中间件模块，比如：    
> `spire_web_http`:定义了基础的http协议类    
> `spire_web_staticfiles`:处理静态资源请求    
> `spire_web_routing`:定义了基础的动态资源路由和流程    
> `spire_web_mvc`:处理模型mvc模式的webapi    
> `spire_extensions_healthchecks`:处理健康检查服务(你可以依赖该模块扩展健康检查项，比如mysql，redis等等)    
> `spire_web_healthchecks`:运行健康检查服务的中间件  
> `spire_web_authoriaztion`:定义了基础的身份认证流程
> `spire_extensions_caching`:定义了缓存标准接口，第三方必须实现该接口，简化asp用户切换存储的压力   
``` cangjie
main(args: Array<String>): Int64 {
    let builder = WebHost.createBuilder(args)
